...
test('all five 5A 7.4 lessons fail closed when a required Part 8 source is unverified', () => {
  for (const source of lessons) {
    for (const sourceId of ['DB4-7.4-14-30', 'SR7-7.1N-3']) {
      const runtime = runtimeOf(source);
      const requiredSource = runtime.sources.find(item => item.id === sourceId);
      assert.ok(requiredSource);
      requiredSource.verification = 'needs-verification';
...
test('all five live 5A 7.4 lesson payloads pass the combined gate with their verified source records', () => {
  for (const source of lessons) {
    const runtime = runtimeOf(source);
    const canonical = validateCanonicalLessonGate(runtime);
