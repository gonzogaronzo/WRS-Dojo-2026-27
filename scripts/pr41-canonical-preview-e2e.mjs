...
async function testPreview() {
  ...
  try {
    ...
    writeReport({ status: 'PASS', previewUrl: PREVIEW_URL, headSha: process.env.GITHUB_SHA || null, qaUid: state.uid, checks, browserErrors });
  } catch (error) {
    await page.screenshot({ path: SCREENSHOT_PATH, fullPage: true }).catch(() => {});
    writeReport({ status: 'FAIL', previewUrl: PREVIEW_URL, headSha: process.env.GITHUB_SHA || null, qaUid: state.uid, checks, browserErrors, error: error instanceof Error ? error.stack || error.message : String(error) });
    throw error;
  } finally {
    await browser.close();
  }
